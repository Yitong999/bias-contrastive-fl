...
from debias.networks.simple_conv_lnl import (LNLSimpleBiasPredictor,
                                             LNLSimpleConvNet)
...
def set_model():
    model = LNLSimpleConvNet().cuda()
    predictor = LNLSimpleBiasPredictor().cuda()
...
def train(train_loader, model, predictor, optimizer, pred_optimizer, epoch, opt):
    ...
    for idx, (images, labels, biases, _) in enumerate(train_iter):
        ...
        images = images.cuda()
        target_logits, bias_feats = model(images)
...
def main():
    ...
    exp_name = f'lnl-color_mnist_corr{opt.corr}-{opt.exp_name}-lr{opt.lr}-bs{opt.bs}-w{opt.weight}-seed{opt.seed}'
