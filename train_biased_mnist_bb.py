...
from debias.losses.bbloss import BBLoss
...
def set_model(train_loader):
    model = SimpleConvNet().cuda()
    criterion = BBLoss(train_loader.dataset.confusion_matrix)
...
def main():
    ...
    exp_name = f'bb-color_mnist_corr{opt.corr}-{opt.exp_name}-lr{opt.lr}-bs{opt.bs}-seed{opt.seed}'
